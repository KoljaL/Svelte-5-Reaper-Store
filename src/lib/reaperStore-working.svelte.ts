...
export class ReaperStore<T> {
     ...
     */
    constructor(key: string, initialValue: T, options?: { serialize?: (val: T) => string; deserialize?: (val: string) => T }) {
        this.#key = key;
        // Reactive counter
        this.#version = 0;
     ...
     */
    static fromLocalStorage<T>(key: string, options?: { serialize?: (val: T) => string; deserialize?: (val: string) => T }): ReaperStore<T> | null {
        try {
            const storedValue = localStorage.getItem(key);
            if (storedValue !== null) {
                const initialValue = options?.deserialize ? options.deserialize(storedValue) : JSON.parse(storedValue);
    ...
    static getAll<T>(prefix: string, options?: { serialize?: (val: any) => string; deserialize?: (val: any) => any }): Record<string, ReaperStore<any>> {
     ...
     */
    static createWithPrefix<T>(prefix: string, key: string, initialValue: T, options?: { serialize?: (val: T) => string; deserialize?: (val: T) => T }): ReaperStore<T> {
